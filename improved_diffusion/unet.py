...
class UNetModel(nn.Module):
    ...
    def _pre_forward(self, x, timesteps, txt=None, attn_mask=None, capt=None, cond_timesteps=None, ):
        ...
        if txt is not None and self.using_capt and capt is not None:
            if not self.cuda_graph_setup_done:
                ...
                x_, attn_mask = self.text_encoder.compute_embeddings_and_mask(txt, timesteps)
                my_attn_mask = th.tile(attn_mask.unsqueeze(1).unsqueeze(1), (self.text_encoder.n_heads, x_.shape[1], 1))
                graph_callable_args_txt = (x_.detach().requires_grad_(True), my_attn_mask)
    ...
    def forward(self, x, timesteps, txt=None, attn_mask=None, capt=None, cond_timesteps=None, ):
        print(f"self.input_blocks[1][0].in_layers[2].weight, {repr(self.input_blocks[1][0].in_layers[2].weight)}")
